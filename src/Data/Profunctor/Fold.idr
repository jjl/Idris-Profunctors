...
export
Semigroup m => Semigroup (L a m) where
  (<+>) = liftA2 (<+>)

export
Monoid m => Monoid (L a m) where
  neutral = pure neutral

export
Group m => Group (L a m) where
  inverse = map inverse

export
AbelianGroup m => AbelianGroup (L a m) where

export
Ring m => Ring (L a m) where
  (<.>) = liftA2 (<.>)

export
RingWithUnity m => RingWithUnity (L a m) where
  unity = pure unity

-- The `Field` implementation won't type check, but it should exist

export
Num n => Num (L a n) where
  (+)         = liftA2 (+)
  (*)         = liftA2 (*)
  fromInteger = pure . fromInteger

export
Neg n => Neg (L a n) where
  (-)         = liftA2 (-)
  negate      = map negate

export
Abs n => Abs (L a n) where
  abs         = map abs
...
export
Semigroup m => Semigroup (R a m) where
  (<+>) = liftA2 (<+>)

export
Monoid m => Monoid (R a m) where
  neutral = pure neutral

export
Num n => Num (R a n) where
  (+)         = liftA2 (+)
  (*)         = liftA2 (*)
  fromInteger = pure . fromInteger

export
Neg n => Neg (R a n) where
  (-)         = liftA2 (-)
  negate      = map negate

export
Abs n => Abs (R a n) where
  abs         = map abs

export
Group m => Group (R a m) where
  inverse = map inverse

export
AbelianGroup m => AbelianGroup (R a m) where

export
Ring m => Ring (R a m) where
  (<.>) = liftA2 (<.>)

export
RingWithUnity m => RingWithUnity (R a m) where
  unity = pure unity
