...
export
Lensing (Forgotten r) where
  strength (Forget ar) = Forget $ ar . snd

export
Functor f => Lensing (UpStarred f) where
  strength (UpStar f) = UpStar . uncurry $ (. f) . (<$>)

export
Lensing Morphism where
...
export
lens' : (Profunctor p, Lensing p) => (s -> (b -> t, a)) -> Lens {p} s t a b
lens' f = lmap {p} f . strength

||| Build a `Lens` out of getter and setter
export
lens : (Profunctor p, Lensing p) => (s -> a) -> (s -> b -> t) -> Lens {p} s t a b
lens gt st = lens' {p} $ \s => (\b => st s b, gt s)
