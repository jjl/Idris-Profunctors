...
export
Functor f => Closed (DownStarred f) where
  closed (DownStar fab) = DownStar $ \fxa,x => fab $ map (\f => f x) fxa

export
Monoid r => Closed (Forgotten r) where
...
public export
record Closure (0 p : Type -> Type -> Type) a b where
...
export
UnsafeProfunctor p => UnsafeProfunctor (Closure p) where
  w         #. (Close p) = Close $ ((.) w) #. p
  (Close p) .# w         = Close $ p       .# ((.) w)

export
Strong p => Strong (Closure p) where
...
export
Profunctor p => Profunctor (Environment p) where
